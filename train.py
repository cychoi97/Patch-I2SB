...
def create_training_options():
    ...
    # optional configs for patch diffusion
    parser.add_argument("--run-patch",      action="store_true",             help="run patch diffusion")
    parser.add_argument("--real-p",         type=float, default=0.5,         help="ratio of full size image")
    ...
    if opt.ckpt is not None:
        ckpt_file = RESULT_DIR / opt.ckpt / f"{opt.start_itr:0>7}.pt"
...
def main(opt):
    ...
    # build dataset
    train_dataset = dataset.Dataset(opt, log, mode='train')
    val_dataset   = dataset.Dataset(opt, log, mode='valid')
