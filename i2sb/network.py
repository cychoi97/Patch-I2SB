...
class Image256Net(torch.nn.Module):
    def __init__(self, log, noise_levels, in_channels=1, use_fp16=False, cond=False, run_patch=False, pretrained_adm=False, ckpt_dir="data/"):
    ...
    def forward(self, x, steps, cond=None, x_pos=None):

        t = self.noise_levels[steps].detach()
        assert t.dim()==1 and t.shape[0] == x.shape[0]

        x = torch.cat([x, cond], dim=1) if self.cond else x
        x = torch.cat([x, x_pos], dim=1) if self.run_patch else x
